...
export const ProfessionalMarketplaceManager: React.FC = () => {
  ...
  const loadProducts = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('target_audience', 'professional')
        .order('created_at', { ascending: false });

      if (error) throw error;
      setProducts(data || []);
    } catch (error) {
      console.error('Error loading professional marketplace products:', error);
      toast({
        title: "Erro",
        description: "Erro ao carregar produtos do marketplace profissional",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving professional marketplace product:', error);
      toast({
        title: "Erro",
  ...
  const handleEdit = (product: ProfessionalProduct) => {
  ...
  if (loading) {
    return <div className="flex justify-center p-8">Carregando produtos para profissionais...</div>;
  }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Insumos, materiais e equipamentos que profissionais podem comprar com saldo da plataforma
          </p>
        </div>
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                ...
                <div className="flex space-x-1">
                  ...
                  <Badge variant="outline">Profissionais</Badge>
                </div>
              </div>
            </CardHeader>
      ...
      {products.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500">Nenhum produto para profissionais cadastrado ainda.</p>
            <p className="text-sm text-gray-400 mt-2">
              Clique em "Cadastrar Produto" para adicionar o primeiro produto do marketplace profissional.
            </p>
          </CardContent>
        </Card>
      )}
