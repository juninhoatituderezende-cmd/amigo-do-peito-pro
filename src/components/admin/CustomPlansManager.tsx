...
export function CustomPlansManager() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Transform plans data with statistics
      const transformedPlans = (plansData || []).map(plan => {
        const groups = plan.plan_groups || [];
        const formingGroups = groups.filter(g => g.status === 'forming').length;
        ...
        return {
          ...
          active: plan.active,
          public_enrollment: plan.public_enrollment,
          total_groups: groups.length,
          forming_groups: formingGroups,
          full_groups: fullGroups,
          completed_groups: completedGroups,
          total_participants: totalParticipants,
          total_revenue: totalParticipants * plan.entry_price,
  ...
  const handleCreatePlan = async () => {
    ...
    setLoading(true);
    try {
      // Create custom plan (plan_code is auto-generated by trigger)
      const { data, error } = await supabase
        ...
        .insert({
          ...
          entry_price: parseFloat(planForm.entry_price),
          max_participants: parseInt(planForm.max_participants),
          professional_id: planForm.professional_id || null,
          allow_professional_choice: planForm.allow_professional_choice,
